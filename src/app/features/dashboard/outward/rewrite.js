...
const tsPath = 'outward.component.ts';
const htmlPath = 'outward.component.html';
...
`;
ts = ts.replace(/this\.sizeBreakdown\.clear\(\);[\s\S]*?this\.calculateTotal\(\);/, patch_form_multi_colour + "\n    this.calculateTotal();");
...
`;
ts = ts.replace(/return this\.selectedCompanyId &&[\s\S]*?this\.outwardForm\.valid;/, is_form_valid);

// Update payload builder logic
ts = ts.replace("colour: formVal.colour,", "colour: formVal.colour || 'MULTI',");
const update_payload_sizes = `
...
`;
ts = ts.replace(/sizeCounts: this\.sizeBreakdown\.getRawValue\(\)\.map\(\(c: any\) => \(\{[\s\S]*?\}\)\)/, update_payload_sizes);

const insert_payload_sizes = `
        colourBreakdowns: this.colourBreakdowns.getRawValue().map((c: any) => ({
          colourName: c.colourName,
          sizes: c.sizes.map((s: any) => ({
            size: s.size,
...
`;
ts = ts.replace(/sizes: this\.sizeBreakdown\.getRawValue\(\)\.map\(\(c: any\) => \(\{[\s\S]*?\}\)\)/, insert_payload_sizes);
...
`;
ts = ts.replace(/sizes: this\.sizeBreakdown\.controls\.map\(c => \(\{[\s\S]*?\}\)\),/, preview_map);


// Save TS
fs.writeFileSync('outward.component.ts', ts, 'utf8');
...
html = html.replace(`<div class="input-group">\n              <label>Colour <span class="required">*</span></label>`, `<div class="input-group" *ngIf="entryType === 'meter'">\n              <label>Colour <span class="required">*</span></label>`);


const multi_colour_html = `
...
`;
html = html.replace(/<!-- Existing: Dynamic Size Breakdown Section \(Size Based only\) -->[\s\S]*?<!-- ── NEW: Meter Based Entry Section/, multi_colour_html + "\n\n          <!-- ── NEW: Meter Based Entry Section");

// Add Colour Picker Modal
const colour_picker_html = `
  <!-- Colour Picker Modal -->
...
fs.writeFileSync('outward.component.html', html, 'utf8');
